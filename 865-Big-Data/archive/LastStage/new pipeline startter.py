...
pipeline = Pipeline(stages=pipelineObj)
pipelineFit = pipeline.fit(trainingData)
import datetime
now = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
pipelineFit.save(f"file:///dbfs/joe/{now}_model_baseline")
...
# pipeline = Pipeline(stages=pipelineObj)
# eda = pipeline.fit(trainingData).transform(trainingData)

# COMMAND ----------

# eda.selectExpr("summary_sentimental_classifications").show(20, False)
